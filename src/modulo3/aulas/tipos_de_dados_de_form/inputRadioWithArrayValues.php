...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if(empty($_POST['payment_option'])){
        ...
        $payment_valid = array_values($formas_pagamento_validate); // Extrai todos os valores de um array e os reindexa
        var_dump($payment_valid);
        var_dump($payment);
        if(!in_array($payment, $payment_valid)){
